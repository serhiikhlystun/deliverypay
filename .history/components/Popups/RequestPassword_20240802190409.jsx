...
const RequestPasswordPopup = ({ isOpen, onClose, csrfToken }) => {
  ...
  const handleSubmit = async e => {
    ...
    const response = await fetch('https://crm.420comrades.com/auth/password/request', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
        body: JSON.stringify({ email, reset_url:"https://420comrades.com/reset" })
    });

    const result = await response.json();
    if (response.ok) {
        document.getElementById('message').textContent = 'Check your email for a password recovery link.';
    } else {
    ...
    <div className={`popup__wrapp ${isOpen ? 'open' : ''}`}>
      ...
      <div className="popup__content">
        <div className="popup__close-btn-wrapp">
          <button className="popup__close-btn" onClick={closeRequestPopup}>
        ...
        <div className="popup__title-wrapp">
          <h2 className="popup__title">RQUEST NEW PASSWORD</h2>
          <p className="popup__subtitle" id="message"></p>
        </div>
