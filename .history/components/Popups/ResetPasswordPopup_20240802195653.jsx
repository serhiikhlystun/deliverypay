...
const ResetPasswordPopup = ({ isOpen, onClose, token }) => {
  ...
  const handleSubmit = async e => {
    ...
    const response = await fetch('https://crm.420comrades.com/auth/password/reset', {
      method: 'POST',
      headers: {
    ...
    if (response.ok) {
      document.getElementById('message').textContent = 'Your password has been reset. You can now log in with your new password.';
      setTimeout(function () {
       onClose();
      },3000)
    } else {
    ...
    <div className={`popup__wrapp ${isOpen ? 'open' : ''}`}>
      ...
      <div className="popup__content">
        ...
        <div className="popup__title-wrapp">
          <h2 className="popup__title">RESET PASSWORD</h2>
