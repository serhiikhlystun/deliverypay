...
Modal.setAppElement('#__next'); // Вкажіть кореневий елемент вашого додатка
...
const ProductItem = ({ product }) => {
  ...
  const renderSlides = (slides, assetsUrl) => {
    return slides.map((slide, index) => (
      <SwiperSlide key={index}>
        <Image
          src={`${assetsUrl}/${slide.directus_files_id.id}${isMobile ? '?width=500&height=500' : '?width=580&height=700'}` }
          width={`${isMobile ? '417' : '580'}`}
    ...
    <section>
      <div className="container">
        <div className="product-item">
          ...
          <ProductSlider product={product} assetsUrl={assetsUrl}></ProductSlider>
      ...
      >
        <Swiper
          ...
          scrollbar={{ draggable: true }}
        >
          {renderSlides(product.slides.length ? product.slides : [{ directus_files_id: { id: product.product_image.id } }], assetsUrl)}
        </Swiper>
        <button type="button" onClick={() => setIsModalOpen(false)} className="modal-close-button">Close</button>
      </Modal>
    </section>
  );
};
